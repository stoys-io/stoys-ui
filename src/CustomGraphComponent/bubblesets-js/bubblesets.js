...
function BubbleSet() {
  ...
  function Rectangle(_rect) {
    ...
    this.intersects = function (rect) {
      if (that.width() <= 0 || that.height() <= 0 || rect.width() <= 0 || rect.height() <= 0)
        return false
      return (
        rect.maxX() > that.minX() &&
        rect.maxY() > that.minY() &&
  ...
  function connectItem(nonMembers, item, visited) {
    ...
    visited.forEach(function (neighbourItem) {
      ...
      if (
        distanceSq * (numberInterferenceItems + 1) * (numberInterferenceItems + 1) <
        minLengthSq
      ) {
        ...
        while (!hasIntersection && linesToCheck.length) {
          ...
          if (closestItem) {
            ...
            if (numIntersections === 2) {
              ...
              var foundFirst =
                pointExists(movePoint, linesToCheck) || pointExists(movePoint, scannedLines)
...
export { BubbleSet, ShapeSimplifier, BSplineShapeGenerator, PointPath }
